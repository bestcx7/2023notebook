...
- 有两种类型的工作负载，一种是计算密集型一种是非计算密集型
- 当训练受计算限制时，训练受限于我们愿意等待的时间，而不是我们拥有的训练数据或者其他因素
  -  在这种情况下，如果我们能够以某种方式训练更长时间或者更有效，我们因该能看见一个更低的训练loss，或者如果适当的调整，能够在验证集上取得更合适的loss
  -  换句话说，加速训练等于提升训练，最佳的训练时间永远是我们负担的起的
  -  这就意味着工作负载训练受限并不意味着训练更长更快是提升结果的唯一办法
- 当训练资源不受限时，我们可以负担的起无论训练多长时间，基于这点，训练更长的时间不会提供任何帮助
  -  在这种情况下，我们应该期待训练到足够低的loss，训练更长时间可能会轻微的减少训练损失，不一定会有意义的减少训练损失
  -  特别是当计算资源不受限时，更慷慨的训练时间预算，使得调整更加容易，特别是当调整学习率衰减时，因为他对训练预算有很强的影响作用
   -  换句话说，特别小的训练时间预算，可能需要学习率衰减变得完美，去达到一个更好的错误率
-  无视给定的工作是否是训练预算受限的，增加梯度方差的训练方法通常会导致训练速度减慢，因此可能会达到特定验证损失的step数，高梯度误差可能是由于以下原因引起的：
   -  使用一个更小的batch size
   -  添加数据增强
   -  添加一些类型的正则化

### 当计算不受限时决定训练多长时间
-  我们的主要目的是训练足够长的时间达到模型最好的可能结果，同时避免在训练step上的过分浪费
-  当有任何疑问时，毫不犹豫的选择训练更长的时间。当回顾性的checkpoint被正确的使用并且被使用的足够频繁时，模型的性能永远不可能下降
-  永远不要调整*max_train_steps*的数量在研究中，选择一个值并且在所有的实验中使用他们，从这些实验中，绘制回顾性checkpoint找到的训练step，从而细化*max_train_steps*的选择
  -  举例来说，如果最佳的训练step在训练的前10%中，那么最大训练steps就太高了
  -  然而，如果最佳的训练step维持在训练的最后25%，那么我们可能会从延长训练时间或者调整衰减计划中收益
-  最佳的训练step数可能会因为结构或者数据的改变而改变
-  接下来我们将要描述如何根据恒定学习率完美拟合训练所需步骤来选择一个*max_train_steps*的候选值
   -  注意，这里的完美拟合只是作为loss非常低的描述
      -  举例来说，当使用对数损失函数，缺少正则化项，如果模型的权重没有限制，我们可能会看到训练loss缓慢提升，直到我们达到浮点数的限制，并且模型在训练集上的预测可能会变得越来越自信，在这种情况下我们可能会说模型完美拟合了训练集，并且在训练集上的错误分类率接近0
   -  我们发现的*max_train_step*可能需要增加，如果梯度噪声随着训练的过程增加
      -  举例来说，如果模型引入了类似于dropout的正则化器
   -  如果训练的过程以某种方式改善*max_train_step*可能需要减少
      -  举例来说，使用了一个更好的优化器或者学习率衰减有了更好的调整
#### 使用学习率扫描为max_trian_step提供初始值的算法

<details><summary><em>[点击进行扩展]</em></summary>

<br>

-  此过程假设可以完美拟合训练集，并且可以使用恒定学习率实现这一点
-  如果能够完美的拟合训练集，那么一定存在一个配置（包括max_train_step的一些值）能够完美拟合训练集，找到这样的配置，并且使用他的max_train_step来作为starting point N
-  运行恒定的学习率扫描（网格搜索学习率），没有数据增强，没有正则话，每个实验训练N个step
-  扫描中最快达到最佳表现的训练的step是我们对max_train_step的猜测
-  **注意**：坏的搜索空间可能会导致自欺欺人
   -  举例来说，如果一个研究中所有的学习率都非常的小，我们可能错误的总结出一个非常大的max_train_step是必要的
   -  至少我们需要检查最佳学习率是否在搜索空间边界

</details>

### 当计算受限时决定训练多长时间
-  在一些情况下，训练loss会无止境的改善，这时候我们的耐心和计算资源就成为了限制
   -  我们也许能够通过运行大量较短的实验进行高效的调整，并且对我们希望运行的实验保持最长的生产长度
   -  随着训练时间接近我们耐心的极限，调整实验和我们预期发布的候选模型越来越相关，但是我们只可以完成他们中的少部分
   -  当我们只训练10%生产长度的实验时，我们可能可以回答很多问题，但可能存在风险，也就是生产长度20%时这些结论就不再适用了，更不用说100%
-  通过每次增加实验的步骤进行多轮调整是一个明智的选择
   - 如果我们想，我们可以做尽可能多轮的实验，但1-3轮是最实用的
   -  本质上，尝试从轮转时间非常快的实验中获取足够多的理解，权衡调优彻底性与最终，最长相关性
   -  一旦一个给定的实验时间限制产生了有用的见解，我们可以增加训练时间并且继续调整，根据需要二次检查我们从短期运行中的得到的结论
-  作为一个开始的点，我们建议两轮调整：
   -  短期运行，找到最好的模型和优化器超参数
   -  在好超参数基础上短期运行得到一个好的模型
-  最大问题就是从第n轮到第n+1轮如何学习率衰减
   -  在轮次之间调整学习率计划，一个常见的问题就是使用了太小学习率的额外训练步骤
...
-  不幸的是，不能保证在短期内找到好的超参数，当训练长度显著增加时，不完整的的训练是一个最佳的选择，然而对于一些超参数，他们有足够的相关性，是他们在第一轮就变得有用
-  什么样的超参数是我们希望在短训练中发现，并且转移到长期训练的，根据我们目前所知，按照转移概率递减的顺序：
   -  非常有可能转移的
      -  早期的训练不稳定可以在第一轮调整中使用几个step解决，可能这些超参数是最接近被转移的
         - 预热长度
         - 初始化
   -  可能被转移
      -  模型架构，模型架构有显著优势的可能被转移，但也有很多反例
   -  可能会转移
      -  优化算法/优化超参数，我们认为这是松散的转移，肯定比上面的弱
      -  数据增强
      -  正则化
         -  如果不能够完美的拟合训练集，那么模型处于的正则化可能不会有太大的帮助
   - 不可能转移的
      -  学习率计划：不可能完美转移
...
- （推测）使用额外的步骤来延长高学习率的训练时间
   -  如果是线性计划，就从第一轮开始，保持固定的衰减长度，并且延长开始时lr的恒定周期
   -  对于余弦衰减来说，从第一轮开始保持基本的lr，并且延长max_train_step
-  对于有非常成熟的建模以及调整pipeline，长且昂贵的生产过程的团队来说，更多的训练轮数可能是有意义的，但通常杀伤力过大
   -  我们已经决定如何从step 1转移到step 2，如果我们不关心分析时间，并且高效的利用计算资源是一个问题，那么理想的情况是在许多不同轮次的调整中延长训练的长度
      -  在每一轮中，我们都要系统的确保我们的选择有效
      -  新的想法通过一个pipe，从step i到step i + 1，逐步增加长训练实验，逐步减低他们的风险
</details>
...
**总结**：输入限制pipeline的原因和措施是高度任务独立的，使用分析器，并查找常见问题  

-  使用合适的分析器来诊断输入绑定的pipeline
-  最终，具体的原因和措施高度依赖于任务，更广泛的工程考虑可能会导致输入pipeline效果更差
-  普通原因：
   -  数据未与训练过程共存，导致I/O延迟
   -  昂贵的在线数据处理
   -  干扰数据pipeline预取的无意同步障碍，
-  普通tips：
   -  用于预取示例的输入pipeline
   -  尽可能早的从数据pipeline中移除元数据或者特征
   -  增加为输入pipeline生成示例的jobs的复制

### 评估模型的表现
**总结**： 在比训练时更大的batchsize上进行验证，以规则的step间隔进行验证而不是规则的时间间隔  
...
-  这里有几个我们可以评估模型表现得设置
   ...
   -  **线下评估** -  当模型在线下训练/验证/测试集上运行时收集指标
   -  **定期评估** -  在模型训练运行期间收集的指标可能是线下评估的代理或者线下评估使用数据的子集
-  线上评估是黄金准则，但在模型生产过程中不切实际
-  取决于不同的问题，线下评估可能复杂且成本昂贵
-  定期评估是最实用，最经济的选择，但无法代表生产环境
   -  我们定期评估的目的是使用离线评估的便捷代理，而不损失训练时信息的可靠性
...
-  我们在训练过程中进行定期评估，以实时监控其进度，促进回顾性模型检查点的选择， 并在训练结束时检查训练曲线。

-  最简单的配置是在同一计算实例中执行训练和周期性评估，在训练和评估之间周期性交替。

   -  在这种情况下，用于执行评估的批量大小应该至少与用于训练的批量大小一样大，因为在评估期间不需要保持模型激活，从而降低了每个示例的计算要求。

-  定期评估应以定期的步骤间隔进行，而不是以时间间隔进行。

   -  基于时间间隔的评估会使解释训练曲线变得更加困难，尤其是当训练可能受到训练作业的抢占、网络延迟问题等的影响时。

-  有效/测试度量的周期性（当使用混洗训练/验证/测试拆分时）可能表明实现错误，例如测试数据与训练数据重叠，或者训练数据没有正确混洗。定期进行评估可以使这些问题更容易发现。

-  当评估集不能被批次大小整除时，可能会出现部分批次。确保填充的示例正确加权，以防止损失函数被它们偏置。通常，这些填充示例的权重可以为零。

-  每次评估都要保存足够的信息，以支持离线分析。理想情况下，我们将保存对个别示例的选择的预测，因为它们对于调试来说是非常宝贵的。

   -  生成SavedModels这样的工件可以在评估作业完成后轻松地进行特别的模型检查。
...
-  定期评估作业的运行速度可能不够快，无法在合理的时间内计算完整脱机评估集的度量。这通常需要采样数据进行定期评估。

-  在构建采样数据集时，我们会考虑以下因素：

   -  样本量

      -  样本量检查定期作业使用的采样数据集上计算的性能是否与整个离线评估集上的性能匹配，即采样集和完整数据集之间没有偏差。

      -  用于定期评估的数据集应该足够小，可以很容易地生成整个模型的预测，但也应该足够大，可以准确地测量模型的改进（即不被标签噪声淹没）。

      -  它应该足够大，可以按顺序在试验中进行多次这样的评估，并且仍然可以产生准确的估计。也就是说，避免随着时间的推移自适应地“拟合”验证集，而不会推广到搁置的测试集。

   -  不平衡的数据集

      -  对于不平衡的数据集，在罕见的示例类上的性能通常是有噪声的。

      -  对于类标签中包含少量示例的数据集，记录正确预测的示例数量，以深入了解准确性的提高（.05灵敏度的提高听起来很令人兴奋，但这只是又一个正确的示例吗？）

</details>


### 保存checkpoint并且回顾选择最佳的checkpoint

**总结**：进行固定步数的训练，并回顾性地从跑步中选择最佳检查点。



-  大多数深度学习框架都支持模型检查点。也就是说，模型的当前状态定期保存在磁盘上。这允许训练作业对计算实例中断具有弹性。

-  最好的检查点通常不是最后一个检查点，特别是当验证集性能没有随着时间的推移而继续增加，而是在特定值附近波动时。

-  建立管道以跟踪培训期间迄今为止看到的N个最佳检查点。在训练结束时，模型选择就是选择训练期间看到的最佳检查点。我们称之为回顾性最佳检查点选择。

-  支持预期的早期停止通常是不必要的，因为我们正在预先指定试验预算，并保留迄今为止看到的N个最佳检查点。
...
-  找一个跟踪系统，它至少可以捕捉上面列出的信息，并且对做这件事的人来说很方便

### batch normalization的执行细节

**摘要**：如今，批处理规范通常可以用LayerNorm代替，但在不能的情况下，在更改批处理大小或主机数量时会有一些棘手的细节。

-  批次规范使用当前批次的平均值和方差对激活进行归一化，但在多设备设置中，除非明确同步，否则每个设备上的这些统计信息是不同的。

-  轶事报道（主要在ImageNet上）称，仅使用约64个例子计算这些归一化统计数据在实践中效果更好（请参阅本文中的Ghost Batch Norm）。

-  将总批量大小和用于计算批量范数统计的示例数解耦，对于批量大小比较特别有用

-  Ghost批处理规范实现并不总是正确处理每设备批处理大小>虚拟批处理大小的情况。在这种情况下，我们实际上需要对每个设备上的批次进行子采样，以获得适当数量的批次范数统计示例。

-  测试模式批处理范数中使用的指数移动平均值只是训练统计数据的线性组合，因此这些EMA只需要在保存到检查点之前进行同步。然而，批处理规范的一些常见实现并不同步这些EMA，而只保存来自第一个设备的EMA。

### 多主机pipeline的注意事项

**总结**：对于日志记录、评估、RNG、检查点和数据分片，多主机训练可以很容易地引入错误！

-  请确保管道仅在一台主机上进行日志记录和检查点设置。

-  请确保在运行评估或检查点之前，跨主机同步批处理规范统计信息。

-  至关重要的是，不同主机之间的RNG种子相同（用于模型初始化），不同主机的种子不同（用于数据混洗/预处理），因此请确保对其进行适当标记。
